...
public class EmployeeView {
	...
	public void insertEmployee() throws Exception{
		...
		Employee emp = new Employee(empId, empName, empNo, email, phone, salary, deptCode, jobCode, salLevel,
				bonus, managerId);
	...
	private void updateEmployee() throws Exception{
		...
		if(result > 0) {
			...
			System.out.println("사번과 일치하는 직원이 존재하지 않습니다.");
		}
	...
	private void deleteEmployee() throws Exception{
		...
		if(input == 'Y') {
			...
			if(result > 0) {
				...
				System.out.println("입력하신 사번은 존재하지 않습니다");
			}
	...
	private void selectDeptEmp() throws Exception{
		...
		List<Employee> list = service.selectDeptEmp(departmentTitle);
	
		if(list.isEmpty()) {
			System.out.println("부서와 일치하는 사원이 없습니다");
		}else {
